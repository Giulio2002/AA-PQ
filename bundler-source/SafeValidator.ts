// Derived from Pimlico Alto v1.2.7 (GPL-3.0); modified 2026-09-10 for AA-PQ.
// See ../licenses/Alto-GPL-3.0.txt and ../THIRD_PARTY.md.
    ...
    implements InterfaceValidator
{
    ...
    async getValidationResultWithTracerV07(
        ...
        // AA-PQ compatibility change, 2026-09-10. Preserve the full validation
        // trace; decode the simulation's normal return rather than an inner
        // DelegateAndRevert frame. Signature/storage/opcode checks stay active.
        const simulation = await this.config.publicClient.call({
            account: zeroAddress,
            to: pimlicoSimulationsAddress,
            data: entryPointSimulationsCallData,
            stateOverride: Object.entries(stateOverrides).map(([address, override]) => ({address: address as Address, ...override}))
        })
        if (!simulation.data) throw new Error("Empty simulation response")
        const validationResult = decodeFunctionResult({
            abi: pimlicoSimulationsAbi,
            functionName: "simulateValidation",
